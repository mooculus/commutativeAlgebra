...
\begin{theorem}[Correspondance Theorem]\label{T:corr}\index{Correspondance Theorem}
...
\begin{definition}\index{Jordan-H\"older chain}\index{composition series} A chain of $A$-modules
\[
M = M_0 \supsetneq M_1\supsetneq \dots \supsetneq M_n =(0)
\]
is a \dfn{Jordan-H\"older chain}, also known as a \dfn{composition series}, if for each $i$, $M_i/M_{i+1}\iso A/\i m$ for some maximal ideal $\i m$ in $A$.
\end{definition}
...
\begin{definition}\index{length}\index{l@$\l$} The \dfn{length} of an $A$-module, denoted by $\l_A(M)$, is the length of a composition series for $M$. That is, if
\[
M = M_0 \supsetneq M_1\supsetneq \dots \supsetneq M_n =(0)
\]
...
\begin{proposition}\index{length!additive property} Given a short exact sequence of $A$-modules
\[
0\to M' \to M\to M'' \to 0
\]
...
\begin{theorem}[Hilbert's Basis Theorem]\index{Hilbert's Basis Theorem} If $A$ is a Noetherian ring, then $A[x]$ is a Noetherian ring.
\end{theorem}
